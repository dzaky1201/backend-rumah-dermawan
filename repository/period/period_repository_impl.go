...
func (repository *PeriodRepositoryImpl) UpdateHavingRelationIncrement(period domain.YearPeriod, havingRelation int) (domain.YearPeriod, error) {
...
func (repository *PeriodRepositoryImpl) UpdateHavingRelationDecrement(period domain.YearPeriod, havingRelation int) (domain.YearPeriod, error) {
	err := repository.db.Model(domain.YearPeriod{}).Where("id = ?", period.Id).Update("have_relation", havingRelation-1).Error
	if err != nil {
		return period, err
	}
